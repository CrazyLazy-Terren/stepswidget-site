...
export async function GET() {
  ...
  const lines = [
    ...
    '- Price: Free to download, with no ads at any tier. The reminders are free, including their settings and schedule. The optional Customization subscription only changes how the widget looks: an active subscription is required to apply a different style, and a style already applied keeps working after the subscription lapses. There is no one-time purchase. See the App Store listing for current amounts.',
